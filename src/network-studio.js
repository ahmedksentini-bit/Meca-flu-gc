...
export function mountStudio(root,navigate){
  ...
  root.querySelectorAll('[data-mode]').forEach(b=>b.onclick=()=>{mode=b.dataset.mode;connectFrom=null;if(mode==='calibrate')calibration=[];root.querySelector('#studioHint').textContent=mode==='connect'?'Cliquer le nœud de départ puis le nœud d’arrivée.':mode==='calibrate'?'Cliquer deux points du plan dont vous connaissez la distance réelle.':'Cliquer pour sélectionner ; déplacer à la souris ou saisir X/Y.';graph();});
  ...
  svg.addEventListener('pointerdown',e=>{const el=e.target.closest('[data-graph-node]');if(!el||mode!=='select')return;remember();drag=project.nodes.find(n=>n.id===el.dataset.graphNode);svg.setPointerCapture(e.pointerId);});
  svg.addEventListener('pointermove',e=>{if(!drag)return;const point=svg.createSVGPoint();point.x=e.clientX;point.y=e.clientY;const loc=point.matrixTransform(svg.getScreenCTM().inverse());drag.x=Math.max(20,Math.min(780,Math.round(loc.x)));drag.y=Math.max(20,Math.min(420,Math.round(loc.y)));graph();});
  svg.addEventListener('pointerup',()=>{if(drag){selected={type:'node',id:drag.id};drag=null;persist();inspector();}});
  svg.addEventListener('pointercancel',()=>{drag=null;persist();});
  ...
  savedList();inspector();update();basemapStatus();loadBasemapImage();
}
