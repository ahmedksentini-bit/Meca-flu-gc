...
const chapterOrder = {
  ...
  hydrostatics: ["HYDRO_LAYERS_01", "HYDRO_MANO_01", "HYDRO_PRESS_03", "FORCE_GATE_02", "FORCE_CIRCULAR_03", "FORCE_PLANE_01", "HYDRO_INCLINED_06", "HYDRO_CURVED_07", "HYDRO_ARCHIMEDE_08", "HYDRO_STABILITY_04", "COMP_DUALGATE_01", "COMP_LOCKDOOR_01"],
...
function home() {
  ...
  const masques = new Set(state.catalog.chapters.filter(ch => ch.hidden).map(ch => ch.id));
  const chapitres = state.catalog.chapters.filter(ch => !ch.hidden);
  const total = state.catalog.exercises.filter(e => !masques.has(e.chapter)).length;
  app.innerHTML = `<section class="hero"><p class="eyebrow">Mécanique des fluides · Génie civil</p><h1>Comprendre, calculer, vérifier.</h1><p>Des exercices paramétriques fidèles au polycopié, avec unités, validation tolérante et correction raisonnée.</p><div class="hero-actions"><button class="hero-software" id="openCalculator"><span>⌁</span> Ouvrir le bureau de calcul</button><small>${calculatorModules.length + advancedModules.length} outils professionnels · résultats instantanés</small></div><div class="signature">École Nationale d’Ingénieurs de Sfax<br><strong>Dr Ahmed Ksentini</strong></div></section><div class="section-title"><div><h2>Ressources du cours</h2><p>Le polycopié, le cours interactif et l’exerciseur à valeurs aléatoires.</p></div></div><section class="resource-grid"><a class="resource" id="resourcePoly" href="docs/Cours_Mecanique_des_Fluides_GC_S1_v01.pdf" rel="noopener"><span class="resource-mark">PDF</span><h3>Polycopié</h3><p>Le cours complet, 126 pages numérotées, lu directement dans l’application.</p></a><a class="resource" href="cours.html"><span class="resource-mark">§</span><h3>Cours interactif</h3><p>Le même cours en ligne : figures, équations composées et calculateurs intégrés.</p></a><a class="resource" href="exerciseur.html"><span class="resource-mark">∑</span><h3>Exerciseur</h3><p>Exercices à valeurs tirées au hasard, correction détaillée et feuille de TD imprimable.</p></a></section><div class="section-title"><div><h2>Choisir un chapitre</h2><p>${total} exercices paramétriques, alignés sur le polycopié du S1.</p></div></div><section class="chapter-grid">${chapitres.map(ch => { const count = exercisesForChapter(ch.id).length; return `<button class="chapter" data-chapter="${ch.id}"><span class="num">${ch.number}</span><h3>${esc(ch.title)}</h3><p>${esc(ch.description)}</p><span class="count">${count} exercice${count>1?"s":""} →</span></button>`; }).join("")}</section>`;
  document.querySelector("#openCalculator").addEventListener("click", () => calculatorPage());
  document.querySelector("#resourcePoly").addEventListener("click", openPdfViewer);
...
async function openPdfViewer(event) {
  ...
  try {
    ...
    // Seules les pages proches de l’écran sont dessinées : un polycopié de cette taille
